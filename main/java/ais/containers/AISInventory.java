...
public class AISInventory implements IInventory {
	public ItemStack[] stackList;
	private Container eventHandler;
	public WeakReference<EntityPlayer> player;
	public boolean blockEvents=false;

	public AISInventory(EntityPlayer player) {
	...
	public ItemStack decrStackSize(int par1, int par2) {
		if (this.stackList[par1] != null) {
			...
			if (this.stackList[par1].stackSize <= par2) {
				...
				if (itemstack != null && itemstack.getItem() instanceof IAIS) {
					((IAIS) itemstack.getItem()).onUnequipped(itemstack,
							player.get());
				}
	...
	public void setInventorySlotContents(int par1, ItemStack stack) {
		...
		if(!blockEvents && this.stackList[par1] != null) {
        	    ((IAIS)stackList[par1].getItem()).onUnequipped(stackList[par1], player.get());
		}
		this.stackList[par1] = stack;
		if (!blockEvents && stack != null && stack.getItem() instanceof IAIS) {
			((IAIS) stack.getItem()).onEquipped(stack, player.get());
		}
		if (eventHandler != null)
			this.eventHandler.onCraftMatrixChanged(this);
		syncSlotToClients(par1);
	...
	public boolean isItemValidForSlot(int i, ItemStack stack) {
		if (stack == null || !(stack.getItem() instanceof IAIS)
				|| !((IAIS) stack.getItem()).canEquip(stack, player.get()))
			return false;
		if (i == 0
				&& ((IAIS) stack.getItem()).getAISType(stack) == AISType.AMULET)
			return true;
		if ((i == 1 || i == 2)
				&& ((IAIS) stack.getItem()).getAISType(stack) == AISType.RING)
			return true;
		if (i == 3
				&& ((IAIS) stack.getItem()).getAISType(stack) == AISType.BELT)
			return true;
		return false;
	}
	...
	public void syncSlotToClients(int slot) {
		try {
			if (ais.proxy.getClientWorld() == null) {
