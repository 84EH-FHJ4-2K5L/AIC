...
public class AISContainerExpanded extends Container
{
    /**
    ...
    public AISContainerExpanded(InventoryPlayer playerInv, boolean par2, EntityPlayer player)
    {
        this.isLocalWorld = par2;
        this.thePlayer = player;
        ais = new AISInventory(player);
        ais.setEventHandler(this);
        if (!player.worldObj.isRemote) {
        	ais.stackList = PlayerHandler.getplayerAIS(player).stackList;
        }
        ...
        this.addSlotToContainer(new AISSlot(ais,AISType.AMULET,0,80,8 + 0 * 18));
        this.addSlotToContainer(new AISSlot(ais,AISType.RING,1,80,8 + 1 * 18));
        this.addSlotToContainer(new AISSlot(ais,AISType.RING,2,80,8 + 2 * 18));
        this.addSlotToContainer(new AISSlot(ais,AISType.BELT,3,80,8 + 3 * 18));
    ...
    public void onContainerClosed(EntityPlayer player)
    {
        ...
        if (!player.worldObj.isRemote) {
        	PlayerHandler.setplayerAIS(player, ais);
        }
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int par2)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            else if (itemstack.getItem() instanceof IAIS && 
            		((IAIS)itemstack.getItem()).getAISType(itemstack)==AISType.AMULET &&
    				((IAIS)itemstack.getItem()).canEquip(itemstack, thePlayer) &&
            ...
            else if (par2>11 && itemstack.getItem() instanceof IAIS && 
            		((IAIS)itemstack.getItem()).getAISType(itemstack)==AISType.RING &&
    				((IAIS)itemstack.getItem()).canEquip(itemstack, thePlayer) &&
            		...
            		!((Slot)this.inventorySlots.get(11)).getHasStack())
            {
                ...
                if (!this.mergeItemStack(itemstack1, j, j + 1, false))
                {
            		...
            		((IAIS)itemstack.getItem()).getAISType(itemstack)==AISType.BELT &&
    				((IAIS)itemstack.getItem()).canEquip(itemstack, thePlayer) &&
        ...
        if (par1ItemStack.isStackable())
        {
            while (par1ItemStack.stackSize > 0 && (!par4 && k < par3 || par4 && k >= par2))
            {
                ...
                if (itemstack1 != null && itemstack1.getItem() == par1ItemStack.getItem() && (!par1ItemStack.getHasSubtypes() || par1ItemStack.getItemDamage() == itemstack1.getItemDamage()) && ItemStack.areItemStackTagsEqual(par1ItemStack, itemstack1))
                {
                    ...
                    {
                    	if (ss instanceof AISSlot) unequipBauble(par1ItemStack);
